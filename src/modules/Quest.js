...
class Quest {
  ...
  */
  #createSuitCell = () => {
    ...
    $suitCell.suitCellElement.classList.add(randomSuit);

    $suitCell.suitCellElement.innerText = Quest.SUIT_SYMBOL[randomSuit] ?? ""
  ...
  */
  #getRandomSuit = () => {
    const randomClassIndex = Math.floor(Math.random() * SUITS.length);

    const randomSuit = SUITS[randomClassIndex]
  ...
  */
  #getSuitGroup = (element) => {
    return this.#suitField.suitCells.filter(cell => cell.classList.contains(element.classList[1]));
  };
    ...
    const processAdjacentElements = (element) => {
      ...
      suitGroup.forEach(suitElement => {
        ...
        if (
          ...
          (
            (suitRect.y === elementRect.y - suitRect.height && suitRect.x === elementRect.x) ||
            (suitRect.y - elementRect.height === elementRect.y && suitRect.x === elementRect.x) ||
            (suitRect.x === elementRect.x - suitRect.width && suitRect.y === elementRect.y) ||
            (suitRect.x - elementRect.width === elementRect.x && suitRect.y === elementRect.y)
          )
        ) {
  ...
  */
  welcom = () => {
    ...
    $app.appendChild(welcome.welcomElement)
  }
